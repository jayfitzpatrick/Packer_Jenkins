...
stages {
  ...
  stage ('Upload Packer Image to Openstack') {
    steps {
      ...
      sh "openstack --insecure image create --disk-format vmdk --file /bitbucket/operating-systems/CentOS7/TemplateBuild/output-vmware-iso/packer-vmware-iso-disk1.vmdk centos-latest"
          }
}
}
post {
